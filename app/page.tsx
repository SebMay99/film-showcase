...
export default function GalleryPage() {
  ...
  const checkAdminKey = () => {
    if (adminKey === process.env.NEXT_PUBLIC_ADMIN_KEY) {
      setAdminMode(true); setShowAdminPrompt(false);
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <button onClick={handleTitleClick} className="font-mono text-[10px] tracking-[0.15em] text-film-brown uppercase select-none cursor-default">
            Roll 01 · {String(photos.length).padStart(2,"0")} frames · ISO 400
          </button>
          {adminMode && <span className="font-mono text-[10px] tracking-[0.1em] text-film-orange border border-film-orange/40 px-2 py-0.5 rounded-sm">ADMIN</span>}
        </div>
        {adminMode && (
        ...
        <div className="fixed inset-0 bg-film-dark/80 z-50 flex items-center justify-center p-4">
          <div className="bg-film-cream border border-film-brown/30 p-6 rounded-sm w-full max-w-sm">
            ...
            <div className="flex gap-2">
              <button onClick={checkAdminKey} className="flex-1 bg-film-dark text-film-cream font-mono text-[11px] tracking-[0.1em] uppercase py-2 rounded-sm hover:bg-film-brown transition-colors">Entrar</button>
              <button onClick={() => setShowAdminPrompt(false)} className="flex-1 border border-film-brown/40 text-film-dark font-mono text-[11px] tracking-[0.1em] uppercase py-2 rounded-sm hover:border-film-brown transition-colors">Cancelar</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-0 border-b border-film-brown/20 mb-8 overflow-x-auto">
        {CATEGORIES.map((cat, i) => (
          <button key={cat} onClick={() => setFilter(cat)}
            className={`font-mono text-[11px] tracking-[0.12em] uppercase px-4 py-2 transition-colors whitespace-nowrap ${i < CATEGORIES.length - 1 ? "border-r border-film-brown/20" : ""} ${filter === cat ? "text-film-dark underline underline-offset-4 decoration-film-brown" : "text-film-brown hover:text-film-orange"}`}>
            {cat}
          </button>
        ))}
      </div>

      {loading && <div className="flex justify-center items-center py-24"><Loader2 size={24} className="animate-spin text-film-brown" /></div>}
        ...
        <div className="columns-2 md:columns-3 gap-3">
          {filtered.map(photo => (
            <div key={photo.id} className="break-inside-avoid mb-3 group">
              <div className="bg-white border border-film-brown/25 p-1.5 pb-7 cursor-pointer relative" onClick={() => setLightbox(photo)}>
                <Image src={photo.public_url} alt={photo.title} width={600} height={400} className="w-full h-auto block film-photo" />
              ...
              <div className="mt-1.5 px-0.5">
                {adminMode
                  ? <input defaultValue={photo.title} onBlur={e => handleTitleEdit(photo, e.target.value)} className="font-serif text-sm italic text-film-dark bg-transparent border-b border-transparent hover:border-film-brown/30 focus:border-film-orange w-full outline-none" />
                  : <p className="font-serif text-sm italic text-film-dark truncate">{photo.title}</p>
                }
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="font-mono text-[10px] text-film-brown/60">{new Date(photo.created_at).toLocaleDateString("es-MX", { year:"numeric", month:"short", day:"numeric" })}</span>
                  {adminMode && <>
                    <select value={photo.category} onChange={e => handleCategoryChange(photo, e.target.value)} onClick={e => e.stopPropagation()} className="font-mono text-[9px] text-film-orange bg-transparent border-none cursor-pointer outline-none uppercase tracking-wide">
                      {CATEGORIES.filter(c => c !== "todos").map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                    <button onClick={() => handleDelete(photo)} className="font-mono text-[9px] text-film-brown/50 hover:text-red-600 transition-colors ml-auto">✕</button>
                  </>}
                </div>
              </div>
            </div>
      ...
      {lightbox && (
        <div className="fixed inset-0 bg-film-dark/92 z-50 flex items-center justify-center p-4 md:p-8" onClick={() => setLightbox(null)}>
          <div className="relative max-w-4xl w-full" onClick={e => e.stopPropagation()}>
            <button onClick={() => setLightbox(null)} className="absolute -top-9 right-0 font-mono text-[11px] tracking-[0.15em] text-film-tan/70 hover:text-film-cream uppercase flex items-center gap-1">
              <X size={12} /> cerrar
            </button>
            <div className="bg-white p-2 pb-10 relative">
              <Image src={lightbox.public_url} alt={lightbox.title} width={1200} height={800} className="w-full h-auto block film-photo" />
              <div className="absolute bottom-2 left-2 right-2 flex justify-between items-end">
                ...
                <p className="font-mono text-[10px] text-film-brown tracking-[0.08em] text-right">
                  {lightbox.frame_num} — {lightbox.category.toUpperCase()}<br />
